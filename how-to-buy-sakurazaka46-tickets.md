...
<H1>如何参与樱坂46现场演出门票FanClub抽选？</H1>

*此指南适用于以中国大陆为长期居留地的居民，致力于帮助樱坂46粉丝通过合规手段看到樱坂46的现场演出。Ver.0.80*
...
- ### **注册Lemino帐号**
  ...
  - 从2025年巡演的东京巨蛋、大阪京瓷巨蛋两场演出开始，同时拥有樱坂46FanClub和Lemino Prime会员的人可以参与Leminoスペシャルシート的抽选。此位置位于内场的前排和花道两侧，属于最好位置，虽然票价为18000日元，但是如果经济上允许的话，还是推荐参与抽选。
  - 登录Lemino需要`dアカウント`,可以直接进入[Docomo官方网站](https://id.smt.docomo.ne.jp/dac/pub/id/conts/register/top/)注册。
    >请注意，注册`dアカウント`需要接收手机验证码，使用Cmlink的话需要联系客服打开日本境外漫游。<br>

  - 注册后在首页选择加入`leminoプレミアム`即可，每月需要支付990日元，首月免费。
- #### **领取Lemino櫻坂46パック** ####
  - 樱坂46FanClub会员可以在[此页面](https://sakurazaka46.com/s/s46/page/Lemino_sakurazaka46pack#campaign)领取三个月的`Lemino櫻坂46パック`。在使用后可以在Lemino免费观看樱坂46的冠名节目，并参与对应的门票抽选。
  - 在进入领取页面后，点击`発行方法はこちら`按钮，会跳转到领取方式教程，在教程的最下方，点击`シリアルコードの発行はこちらから`按钮，即可根据流程领取`Lemino櫻坂46パック`，领取前提为需要先注册`dアカウント`。
...
- #### **参与Lemino抽选** ####
  - Lemino抽选与FC抽选的优先级相同，分为`Leminoスペシャルシート先行`
和`Lemino櫻坂46パック先行`两种，这两种优先级也并列，与FC先行一样互不冲突。同时也都需要FC帐号。等于新增了两次抽选机会，对于想要看的演出，强烈建议三种抽选全都参与。
  - Lemino抽选入口与FanClub抽选入口在同一页面。从入口进入会首先进入Lemino的对应页面，然后可以选择`Leminoスペシャルシート先行`或`Lemino櫻坂46パック先行`，之后会验证你的Lemino帐号和FC帐号是否有抽选权限，验证通过后同FacClub抽选一样，会进入罗森票务的界面。之后流程与FanClub抽选相同。
    >Leminoスペシャルシート抽选无法选择同行者。<br>


<br>

- ### **参与Official先行抽选**（此条目待更新）
  - 假如在FanClub那一轮的抽选中没有抽到票，作为海外粉丝还可以参与Official先行抽选。
  - Official先行抽选使用罗森票务系统，需要持有罗森帐号。
